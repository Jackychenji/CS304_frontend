...
import {IssueEditRequestData} from "@/api/types/StudyAdvice/editIssue.ts";
import {IssueEditApi} from "@/api/modules/StudyAdvice/editIssue.ts";
import {IssueOpenApi} from "@/api/modules/StudyAdvice/openIssue.ts";
import {IssueCloseApi} from "@/api/modules/StudyAdvice/closeIssue.ts";
import {deleteIssueApi} from "@/api/modules/StudyAdvice/deleteIssue.ts";
...
const editIssue = (issue: number) => {
  const editIssue: IssueEditRequestData = {
    title: title.value,
    issueId: issue,
...
const handleChange = () => {
  if (open.value) {
    IssueOpenApi(issueId)
        .then(response => {
          console.log("reopen successful:", response);
          alert('修改成功')
          getIssueData();
        })
        .catch(error => {
    ...
    IssueCloseApi(issueId)
        .then(response => {
          console.log("close issue successful:", response);
          alert('修改成功')
          getIssueData();
        })
        .catch(error => {
...
<template>
  <el-container style="width: 1410px">
    ...
    >
      <template #header>
        <el-switch
            ...
            @change="handleChange"
        ></el-switch>
        <el-button style="margin-left: 10px" size="large" type="danger" @click="deleteIssue()">删除</el-button>
      </template>
      <a-typography>
        <a-typography-title>{{ selectedRow?.title }} <span class="issue-number">#{{ selectedRow?.issueId }}</span>
          <el-button type="primary" size="large" style="margin-left: 10px" @click="dialogVisible = true">
        ...
        <div v-for="sol in solution" :key="sol.solutionId">
          ...
          >
            <template #actions>
              ...
              <el-button size="small" type="danger" @click="deleteSolution(sol.solutionId)">删除</el-button>
            </template>
            <template v-if="showReplyInput === sol.solutionId">
              <el-input type="text" v-model="replyContent" style="width: 200px"
            ...
            <template v-for="disc in discussion[sol.solutionId]" :key="disc.discussionId">
              ...
              <span key="comment-nested-reply-to" v-if="selectedRow?.open"
                  ...
                  <el-button size="small" type="danger" @click="deleteDiscussion(disc.discussionId)">删除
                  </el-button>
                </template>
                <template v-if="showReplyInput === disc.discussionId">
                  <el-input type="text" v-model="replyContent" style="width: 200px"
                            placeholder="Type your discussion here"/>
                  <el-button type="primary" text bg size="small" @click="postReply()">上传</el-button>
                </template>
              </a-comment>
    ...
    <el-dialog v-model="dialogVisible" title="修改信息" width="40%">
      <!-- Dialog content -->
      <div class="evaluation-dialog-content">
        <a-input addon-before="问题标题" v-model:value="title" placeholder="Basic usage" size="large"/>
        <a-input addon-before="问题内容" v-model:value="content" placeholder="Basic usage" size="large"/>
      </div>
      <!-- Dialog footer -->
      <template #footer>
        <el-button @click="dialogVisible = false">取消</el-button>
        <el-button type="primary" @click="editIssue(issueId)" :loading="loading">确认</el-button>
      </template>
    </el-dialog>
  </el-container>
</template>
